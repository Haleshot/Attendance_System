...
from PyQt5.QtWidgets import QWidget, QApplication, QMainWindow, QMessageBox, QFileDialog, QTableWidgetItem, QTableWidget
...
# from PIL import Image # For generation and storing of images.
import pandas as pd # Reading of CSV File.
import datetime # To display respective Date and Time for Attendance log.
import time # For obtaining the current time -> essential information for Attendance Log.
...
"padding-right:20px;")
    ...
    def TakeImages(self):
        ...
        if exists:
            with open("StudentDetails\StudentDetails.csv", 'r') as csvFile1:
                ...
                for l in reader1:
                    serial = serial + 1
            serial = (serial // 2)
            csvFile1.close() # Closs the CSV File once read
        else:
        ...
        if len(Id) == 0 or len(name) == 0:  # Checking whether the Value of the ID and Name entered by the User in the GUI Window Text Box is Empty or not.
            ...
            if ((name.isalpha()) or (' ' in name)):
                ...
                harcascadePath = "Project_Materials\haarcascade_frontalface_default.xml"
    ...
    def TrackImages(self):
        ...
        exists3 = os.path.isfile("TrainingImageLabel\Trainner.yml") # Checking whether the Trained Model exists.
        if exists3:
        ...
        if exists1:
            ...
            cam.release()
            cv2.destroyAllWindows() # Destroys the Camera Window prompt
        while True:
            ret, im = cam.read()
            ...
            if (cv2.waitKey(1) == ord('q')): # Exits the prompt camera window when User hits the q key on the keyboard.
                break
        ts = time.time() # Generating the Current Time for Attendance Log.
        date = datetime.datetime.fromtimestamp(ts).strftime('%d-%m-%Y')
        ...
        with open("Attendance\Attendance_" + date + ".csv", 'r') as csvFile1:
            ...
            for lines in reader1:
                i = i + 1 # There exists a line in the csv file to read values from.
                if (i > 1):
                    if (i % 2 != 0):
                        iidd = str(lines[0]) + '   '
        ...
        csvFile1.close()
        cam.release()
        cv2.destroyAllWindows() # Destroying the windows once the user has pressed 
